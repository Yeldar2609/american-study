...
async function setTemporaryAdminLanguage(locale: Locale) {
  ...
  const { error } = await admin.from("users").update({ language: locale }).eq("id", temporaryAdminId)
...
test.describe.serial("admin analytics dashboard", () => {
  test.beforeAll(async () => {
    ...
    try {
      ...
      const demotion = await admin.from("users").update({ role: "parent" }).eq("id", originalAdmin.id)
